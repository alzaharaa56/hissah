...
/**
 * ============================================================================
 * Public Pages & Authentication Module
 * ============================================================================
 * Handles public-facing views (Landing, About, Opportunities list) and user 
 * authentication workflows (Login and Registration) for the Hissah platform.
 */
...
const listOf = (d) =>
  Array.isArray(d) ? d : d?.content || d?.items || d?.data || [];
const value = (o, ...keys) => {
  for (const k of keys)
...
const pageHeader = (title, subtitle = "", actions = "") =>
  `<div class="page-header"><div><h1>${esc(title)}</h1><p class="muted">${esc(subtitle)}</p></div><div class="row wrap">${actions}</div></div>`;
const err = (el, e) => {
  if (el)
    el.innerHTML = `<div class="card"><h3>Unable to load this section</h3><p class="muted">${esc(e.message)}</p><button class="btn btn-secondary" onclick="location.reload()">Try again</button></div>`;
  toast(e.message, "error");
};
const opportunityCard = (o) =>
  `<article class="card hover opportunity-card"><div class="row-between"><span class="eyebrow">${esc(value(o, "categoryName", "category", "sector") || "Subcontracting")}</span>${badge(value(o, "status") || "OPEN")}</div><div><h3>${esc(value(o, "title") || "Work Package")}</h3><p class="muted">${esc((value(o, "scope", "description") || "No scope description supplied.").slice(0, 150))}</p></div><div class="opportunity-meta"><span class="badge">📍 ${esc(value(o, "location") || "Oman")}</span><span class="badge">💰 ${money(value(o, "budgetMax", "budgetMin") || 0)}</span><span class="badge">📅 ${date(value(o, "deadline"))}</span></div><div class="opportunity-footer"><small class="muted">${esc(value(o, "eligibilityType") || "OPEN")}</small><a class="btn btn-primary btn-sm" href="${page("public/opportunity-details.html")}?id=${value(o, "id") || ""}">View details</a></div></article>`;
export async function landing() {
  mountPublic(
    `<section class="hero"><div class="container hero-grid"><div><span class="eyebrow">Empowering Omani construction partnerships</span><h1>Transparent subcontracting, <em class="text-primary">rooted in opportunity.</em></h1><p>Hissah connects main contractors with qualified Omani SMEs, bringing project packages, bids, awards and milestone progress into one organized platform.</p><div class="hero-actions"><a class="btn btn-primary" href="${page("public/opportunities.html")}">Explore opportunities</a><a class="btn btn-secondary" href="${page("public/register.html")}">Join Hissah</a></div></div><div class="hero-art"><img src="${asset("images/hero.svg")}" alt="Omani construction collaboration illustration"></div></div></section><section class="section section-soft"><div class="container"><div style="text-align:center;max-width:720px;margin:0 auto 35px"><h2>A complete subcontracting workflow</h2><p class="muted">From company verification to opportunity publication, bid evaluation, award and milestone tracking.</p></div><div class="grid grid-4">${[
      [
        "01",
        "Publish",
        "Main contractors create projects and focused work packages.",
      ],
      ["02", "Discover", "Verified SMEs filter suitable opportunities."],
      ["03", "Bid", "Subcontractors submit structured commercial proposals."],
      ["04", "Deliver", "Awards and milestones keep progress visible."],
    ]
      .map(
        (x) =>
          `<div class="card"><div class="feature-icon">${x[0]}</div><h3>${x[1]}</h3><p class="muted">${x[2]}</p></div>`,
      )
      .join(
        "",
      )}</div></div></section><section class="section"><div class="container grid grid-2"><div class="card soft"><span class="eyebrow">For main contractors</span><h2>Source capable local partners</h2><p>Break projects into clear packages, compare submissions fairly and record award decisions.</p><a class="btn btn-primary" href="${page("public/register.html")}?role=MAIN_CONTRACTOR">Register as contractor</a></div><div class="card soft"><span class="eyebrow">For SMEs</span><h2>Find work that matches your capability</h2><p>Search open packages, submit bids and track your awards from one dashboard.</p><a class="btn btn-amber" href="${page("public/register.html")}?role=SUBCONTRACTOR">Register as SME</a></div></div></section>`,
  );
}
export async function about() {
  mountPublic(
    `<section class="section"><div class="container"><span class="eyebrow">About Hissah</span><div class="grid grid-2" style="align-items:center;margin-top:20px"><div><h1>A digital share of opportunity for Omani SMEs.</h1><p class="muted">Hissah — حِصَّة — means a share or portion. The platform gives qualified local businesses clearer access to subcontracting work while giving main contractors a structured way to source, evaluate and manage partners.</p><div class="grid grid-2"><div class="card"><strong>Transparent</strong><p class="muted">Standardized packages and bid information.</p></div><div class="card"><strong>Local</strong><p class="muted">Built around Omani companies, CRs and Riyada-focused eligibility.</p></div></div></div><img class="hero-art" src="${asset("images/hero.svg")}" alt="Construction collaboration"></div></div></section><section class="section section-soft"><div class="container"><h2>How the platform works</h2><div class="grid grid-3"><div class="card"><h3>1. Verify</h3><p>Companies register, complete profiles and upload documents for administrator review.</p></div><div class="card"><h3>2. Collaborate</h3><p>Contractors publish project packages and SMEs submit commercial and technical bids.</p></div><div class="card"><h3>3. Track</h3><p>Awards, milestones, notifications and reports keep delivery organized.</p></div></div></div></section>`,
  );
}
export async function login() {
  const content = `<div class="auth-page"><section class="auth-art"><a class="brand" href="${page("index.html")}"><img src="${asset("logo/hissah-logo.svg")}" alt="Hissah"></a><div><h1>Welcome back.</h1><p>Continue managing projects, bids and awards with confidence.</p></div><img src="${asset("images/hero.svg")}" alt=""></section><section class="auth-form-wrap"><div class="auth-form"><span class="eyebrow">Secure account access</span><h2>Login to Hissah</h2><p class="muted">Use the email and password connected to your account.</p><form id="login-form" class="stack"><div class="input-group"><label>Email</label><input class="input" type="email" name="email" required></div><div class="input-group"><label>Password</label><input class="input" type="password" name="password" required></div><button class="btn btn-primary btn-block">Login</button></form><p>New to Hissah? <a class="text-primary" href="${page("public/register.html")}"><strong>Create an account</strong></a></p></div></section></div>`;
  mountAuth(content);
  qs("#login-form").addEventListener("submit", async (e) => {
    ...
    try {
      const data = await api.auth.login(formObject(e.target));
      session.set({
      ...
      location.href = dashboardPath(data.role || data.user?.role);
    } catch (ex) {
...
export async function opportunities() {
  mountPublic(
    `<section class="section"><div class="container">${pageHeader("Browse Opportunities", "Search open subcontracting packages across Oman.")}<div class="card filters"><input class="input" id="keyword" placeholder="Search title or scope"><select class="select" id="location"><option value="">All locations</option><option>Muscat</option><option>Sohar</option><option>Salalah</option><option>Nizwa</option><option>Duqm</option></select><select class="select" id="eligibility"><option value="">All eligibility</option><option>OPEN</option><option>SME_ONLY</option><option>RIYADA_PREFERRED</option><option>RIYADA_REQUIRED</option></select><button class="btn btn-primary" id="search">Search</button></div><div id="opportunities" class="grid grid-3" style="margin-top:24px"></div></div></section>`,
  );
  const load = async () => {
    const el = qs("#opportunities");
    loading(el);
    try {
      const d = await api.packages.search({
        keyword: qs("#keyword").value,
        location: qs("#location").value,
        eligibilityType: qs("#eligibility").value,
        status: "OPEN",
        page: 0,
        size: 30,
        sortBy: "publishedAt",
...
export async function userProfile() {
  ...
  try {
    const u = await api.user.me(session.user().id);
    Object.entries(u).forEach(([k, v]) => {
      const input = qs(`[name=${k}]`);
  ...
  qs("#user-form").addEventListener("submit", async (e) => {
    e.preventDefault();
    try {
      const d = await api.user.update(session.user().id, {
        fullName: e.target.fullName.value,
        phone: e.target.phone.value,
      });
      session.set({ ...session.get(), user: { ...session.user(), ...d } });
...
export async function companyProfile() {
    ...
    try {
      ...
      el.innerHTML = l.length
        ...
        : empty(
            ...
            "Upload the Commercial Registration to begin verification.",
          );
      qsa(".delete-doc").forEach(
        (b) =>
          (b.onclick = async () => {
            if (confirm("Delete this document?")) {
              await api.documents.remove(b.dataset.id);
              loadDocs();
            }
          }),
      );
  ...
  try {
    const c = await api.company.me(session.user().id);
    session.set({ ...session.get(), company: c });
    Object.entries(c).forEach(([k, v]) => {
      const i = qs(`[name=${k}]`);
      if (i) i.value = v ?? "";
    });
    qs("#verification-status").innerHTML = badge(c.verificationStatus);
  } catch (e) {
    toast(e.message, "error");
  }
  qs("#company-form").onsubmit = async (e) => {
    e.preventDefault();
    try {
      const c = await api.company.update(cid, formObject(e.target));
      session.set({ ...session.get(), company: c });
  ...
  qs("#document-form").onsubmit = async (e) => {
    e.preventDefault();
    try {
      await api.documents.upload(cid, new FormData(e.target));
...
export async function notifications() {
  if (!requireAuth()) return;
  mountApp("Notifications");
  const root = qs("#page-root");
  root.innerHTML =
    pageHeader(
      "Notifications",
      "Updates about verification, bids, awards and milestones.",
      '<button class="btn btn-secondary" id="read-all">Mark all as read</button>',
    ) + `<div class="card" id="notifications"></div>`;
  const load = async () => {
    const el = qs("#notifications");
    loading(el);
    try {
      const l = listOf(await api.notifications.list());
      el.innerHTML = l.length
        ? l
            .map(
              (n) =>
                `<div class="notification ${n.read ? "" : "unread"}"><div class="notification-icon">●</div><div style="flex:1"><div class="row-between"><strong>${esc(n.title || n.type || "Notification")}</strong><small>${date(n.createdAt, true)}</small></div><p>${esc(n.message || "")}</p>${n.read ? "" : `<button class="btn btn-secondary btn-sm mark-read" data-id="${n.id}">Mark read</button>`}</div></div>`,
            )
            .join("")
        : empty(
...
export async function myAwards() {
  if (!requireAuth(["MAIN_CONTRACTOR", "SUBCONTRACTOR"])) return;
  mountApp("My Awards");
  const root = qs("#page-root");
  root.innerHTML =
    pageHeader(
      "Awards & Milestones",
      "Track active agreements and delivery progress.",
    ) + `<div id="awards" class="grid grid-2"></div>`;
  const el = qs("#awards");
  loading(el);
  try {
    const l = listOf(await api.awards.mine());
    el.innerHTML = l.length
      ? l
          .map(
            (a) =>
              `<article class="card hover"><div class="row-between"><h3>Award #${a.id}</h3>${badge(a.status)}</div><p class="muted">Work package ${esc(a.workPackageTitle || "#" + (a.workPackageId || "—"))}</p><div class="grid grid-2"><div><small>Agreed amount</small><strong style="display:block">${money(a.agreedAmount)}</strong></div><div><small>Delivery</small><strong style="display:block">${esc(a.agreedDeliveryDays || "—")} days</strong></div></div><a class="btn btn-primary btn-block" style="margin-top:18px" href="${page("shared/award-details.html")}?id=${a.id}">Manage award</a></article>`,
          )
          .join("")
      : empty("No awards yet", "Awards appear after a bid is selected.");
  } catch (e) {
    err(el, e);
  }
}
export async function awardDetails() {
  if (!requireAuth(["MAIN_CONTRACTOR", "SUBCONTRACTOR"])) return;
  mountApp("Award Details");
  const root = qs("#page-root");
  root.innerHTML = `<div id="award"></div><section class="card" style="margin-top:22px"><div class="row-between"><div><h2>Milestones</h2><p class="muted">Track work from not started through approval.</p></div>${session.role() === "MAIN_CONTRACTOR" ? '<button class="btn btn-primary" id="new-milestone">Add milestone</button>' : ""}</div><div id="milestones"></div></section><div id="modal"></div>`;
  const aid = idParam();
  try {
    const a = await api.awards.get(aid);
    qs("#award").innerHTML =
      pageHeader(`Award #${a.id}`, a.workPackageTitle || "Award agreement") +
      `<div class="grid grid-4"><div class="card stat"><div><small>Status</small><div>${badge(a.status)}</div></div></div><div class="card stat"><div><small>Agreed amount</small><strong>${money(a.agreedAmount)}</strong></div></div><div class="card stat"><div><small>Delivery days</small><strong>${esc(a.agreedDeliveryDays || 0)}</strong></div></div><div class="card stat"><div><small>Awarded</small><strong style="font-size:1rem">${date(a.awardedAt || a.createdAt)}</strong></div></div></div>`;
  } catch (e) {
    err(qs("#award"), e);
  }
  const load = async () => {
    const el = qs("#milestones");
    loading(el);
    try {
      const l = listOf(await api.milestones.list(aid));
      el.innerHTML = l.length
        ...
        : empty(
            "No milestones",
            "The main contractor can add the first delivery milestone.",
          );
      wireMilestones();
    } catch (e) {
      err(el, e);
  ...
  function modal(html) {
    ...
    qs(".close-modal").onclick = () => (qs("#modal").innerHTML = "");
  }
  function wireMilestones() {
    qsa(".submit-ms").forEach(
      (b) =>
        (b.onclick = async () => {
          try {
            await api.milestones.submit(b.dataset.id);
            toast("Milestone submitted");
            load();
          } catch (e) {
            toast(e.message, "error");
          }
    ...
    qsa(".approve-ms").forEach(
      (b) =>
        (b.onclick = async () => {
          try {
            await api.milestones.approve(b.dataset.id);
            toast("Milestone approved");
            load();
          } catch (e) {
            toast(e.message, "error");
          }
...
export async function contractorDashboard() {
  ...
  mountApp("Contractor Dashboard");
  const root = qs("#page-root");
  root.innerHTML =
    pageHeader(
      "Contractor Dashboard",
      "A clear view of projects, packages, bids and awards.",
      `<a class="btn btn-primary" href="${page("contractor/project-form.html")}">Create project</a>`,
    ) +
    `<div id="stats" class="grid grid-4"></div><div class="grid grid-2" style="margin-top:22px"><section class="card"><div class="row-between"><h2>Recent projects</h2><a href="${page("contractor/projects.html")}" class="text-primary">View all</a></div><div id="recent-projects"></div></section><section class="card"><h2>Recent activity</h2><div id="recent-activity"></div></section></div>`;
  try {
    const [d, p, n] = await Promise.all([
      api.reports.dashboard(),
      api.projects.mine(session.company().id, 0, 5),
      api.notifications.recent().catch(() => []),
    ]);
    const stats = [
      ["Projects", d.totalProjects],
      ["Work Packages", d.totalWorkPackages],
      ["Submitted Bids", d.submittedBids],
      ["Active Awards", d.activeAwards],
    ];
    qs("#stats").innerHTML = stats
      .map(
        (x, i) =>
          `<div class="card stat"><div><small class="muted">${x[0]}</small><strong>${x[1] ?? 0}</strong></div><div class="stat-icon">${["▦", "▤", "✎", "✓"][i]}</div></div>`,
      )
      .join("");
    const l = listOf(p);
    qs("#recent-projects").innerHTML = l.length
      ? l
          .map(
            (x) =>
              `<div class="notification"><div class="notification-icon">▦</div><div><a href="${page("contractor/project-details.html")}?id=${x.id}"><strong>${esc(x.title)}</strong></a><div>${badge(x.status || "DRAFT")} <small>${esc(x.referenceNumber || "")}</small></div></div></div>`,
          )
          .join("")
      : empty("No projects yet", "Create your first main-contract project.");
    const nl = listOf(n);
    qs("#recent-activity").innerHTML = nl.length
      ? nl
          .slice(0, 5)
          .map(
            (x) =>
              `<div class="notification"><div class="notification-icon">●</div><div><strong>${esc(x.title || x.type)}</strong><p class="help">${esc(x.message || "")}</p></div></div>`,
          )
          .join("")
      : empty("No recent activity", "Workflow notifications will appear here.");
  } catch (e) {
    toast(e.message, "error");
  }
}
export async function projects() {
  if (!requireAuth(["MAIN_CONTRACTOR"])) return;
  mountApp("My Projects");
  const root = qs("#page-root");
  root.innerHTML =
    pageHeader(
      "My Projects",
      "Create and manage main-contract projects.",
      `<a class="btn btn-primary" href="${page("contractor/project-form.html")}">New project</a>`,
    ) + `<div id="projects" class="grid grid-3"></div>`;
  const el = qs("#projects");
  loading(el);
  try {
    const l = listOf(await api.projects.mine(session.company().id, 0, 50));
    el.innerHTML = l.length
      ? l
          .map(
            (p) =>
              `<article class="card hover"><div class="row-between">${badge(p.status || "DRAFT")}<small>${esc(p.referenceNumber || "")}</small></div><h3>${esc(p.title)}</h3><p class="muted">📍 ${esc(p.location || "—")} · ${esc(p.sector || "OTHER")}</p><p>${date(p.startDate)} – ${date(p.endDate)}</p><a class="btn btn-primary btn-block" href="${page("contractor/project-details.html")}?id=${p.id}">Open project</a></article>`,
          )
          .join("")
      : empty(
          "No projects created",
          "Create a project before adding work packages.",
        );
  } catch (e) {
    err(el, e);
  }
...
export async function projectForm() {
  ...
  qs("#project-form").onsubmit = async (e) => {
    e.preventDefault();
    try {
      ...
      location.href = `${page("contractor/project-details.html")}?id=${d.id}`;
    } catch (ex) {
      toast(ex.message, "error");
    }
  };
...
export async function projectDetails() {
  if (!requireAuth(["MAIN_CONTRACTOR"])) return;
  mountApp("Project Details");
  const root = qs("#page-root"),
    id = idParam();
  root.innerHTML = `<div id="project"></div><section class="card" style="margin-top:22px"><div class="row-between responsive"><div><h2>Work Packages</h2><p class="muted">Break this project into packages for subcontractor bidding.</p></div><a class="btn btn-primary" href="${page("contractor/work-package-form.html")}?projectId=${id}">Create package</a></div><div id="packages"></div></section>`;
  try {
    const p = await api.projects.get(id, session.company().id, session.role());
    qs("#project").innerHTML =
      pageHeader(
        p.title,
        p.referenceNumber || "",
        `<a class="btn btn-secondary" href="${page("contractor/project-form.html")}?id=${p.id}">Edit</a><button class="btn btn-primary project-action" data-action="activate">Activate</button><button class="btn btn-secondary project-action" data-action="complete">Complete</button>`,
      ) +
      `<div class="grid grid-4"><div class="card"><small>Status</small><div>${badge(p.status || "DRAFT")}</div></div><div class="card"><small>Sector</small><strong style="display:block">${esc(p.sector)}</strong></div><div class="card"><small>Location</small><strong style="display:block">${esc(p.location)}</strong></div><div class="card"><small>Timeline</small><strong style="display:block;font-size:.9rem">${date(p.startDate)} – ${date(p.endDate)}</strong></div></div><div class="card" style="margin-top:22px"><h3>Project description</h3><p>${esc(p.description || "No description")}</p></div>`;
    qsa(".project-action").forEach(
      (b) =>
        (b.onclick = async () => {
          try {
            ...
            toast(`Project ${b.dataset.action}d`);
            location.reload();
          } catch (e) {
            toast(e.message, "error");
          }
  ...
  const el = qs("#packages");
  loading(el);
  try {
    const l = listOf(await api.packages.byProject(id));
    el.innerHTML = l.length
      ...
      : empty("No work packages", "Create the first package for this project.");
    qsa(".package-action").forEach(
      (b) =>
        (b.onclick = async () => {
          try {
            await api.packages.action(b.dataset.id, b.dataset.action);
  ...
  try {
    ...
    qs("#category-select").innerHTML =
      ...
      cats
        ...
        .join("");
    if (id) {
      const d = await api.packages.get(id);
      Object.entries(d).forEach(([k, v]) => {
        const i = qs(`[name=${k}]`);
        if (i)
          i.value = k === "deadline" && v ? String(v).slice(0, 16) : (v ?? "");
      });
    }
  } catch (e) {
    toast(e.message, "error");
  }
  qs("#package-form").onsubmit = async (e) => {
    e.preventDefault();
    const b = formObject(e.target);
    b.categoryId = Number(b.categoryId);
    b.projectId = Number(b.projectId);
    b.budgetMin = Number(b.budgetMin);
    b.budgetMax = Number(b.budgetMax);
    try {
      const d = id
        ? await api.packages.update(id, b)
        : await api.packages.create(projectId, b);
      toast(id ? "Package updated" : "Package created");
      location.href = `${page("contractor/project-details.html")}?id=${d.projectId || projectId}`;
    } catch (ex) {
      toast(ex.message, "error");
    }
  };
...
export async function compareBids() {
  ...
  const root = qs("#page-root"),
    wp = idParam("workPackageId");
  root.innerHTML =
    pageHeader(
      "Compare Bids",
      "Review submitted proposals and make a transparent award decision.",
    ) + `<div id="bids"></div><div id="modal"></div>`;
  const el = qs("#bids");
  loading(el);
  try {
    const l = listOf(await api.bids.forPackage(wp));
    el.innerHTML = l.length
    ...
    qsa(".bid-action").forEach(
      (b) =>
        (b.onclick = async () => {
          const reason = prompt(`Reason for ${b.dataset.action}:`) || "";
          try {
            await api.bids.action(b.dataset.id, b.dataset.action, { reason });
            toast(`Bid ${b.dataset.action}ed`);
            location.reload();
          } catch (e) {
            toast(e.message, "error");
          }
    ...
    qsa(".award-bid").forEach(
      (b) =>
        (b.onclick = () => {
          ...
          qs("#award-form").onsubmit = async (e) => {
            e.preventDefault();
            const f = formObject(e.target);
            try {
              const a = await api.awards.create(wp, f.bidId, {
                agreedAmount: Number(f.agreedAmount),
                agreedDeliveryDays: Number(f.agreedDeliveryDays),
                notes: f.notes,
              });
              toast("Bid awarded");
              location.href = `${page("shared/award-details.html")}?id=${a.id}`;
            } catch (ex) {
              toast(ex.message, "error");
            }
          };
...
export async function reports() {
  if (!requireAuth(["MAIN_CONTRACTOR"])) return;
  mountApp("Reports & Analytics");
  const root = qs("#page-root");
  root.innerHTML =
    pageHeader(
      "Reports & Analytics",
      "Operational totals and recent platform activity.",
    ) + `<div id="report"></div>`;
  const el = qs("#report");
  loading(el);
  try {
    ...
    const metrics = [
      ...
      ["Packages", d.totalWorkPackages || 0],
      ["Open Packages", d.openWorkPackages || 0],
      ["Bids", d.totalBids || 0],
      ["Submitted Bids", d.submittedBids || 0],
      ["Active Awards", d.activeAwards || 0],
      ["Pending Milestones", d.pendingMilestones || 0],
      ["Unread Notifications", d.unreadNotifications || 0],
    ];
    el.innerHTML = `<div class="grid grid-4">${metrics.map((m) => `<div class="card stat"><div><small>${m[0]}</small><strong>${m[1]}</strong></div></div>`).join("")}</div><div class="grid grid-2" style="margin-top:22px"><div class="card"><h2>Activity overview</h2><div class="chart-bars">${metrics
      .slice(0, 6)
      .map(
        (m, i) =>
          `<div class="chart-bar" style="height:${Math.max(18, Math.min(100, Number(m[1]) * 12 + 20))}%"><span>${m[0].split(" ")[0]}</span></div>`,
      )
      .join(
        "",
      )}</div></div><div class="card"><h2>What to watch</h2><div class="timeline"><div class="timeline-item"><strong>Open packages</strong><p class="muted">Close or award packages after evaluation.</p></div><div class="timeline-item"><strong>Pending milestones</strong><p class="muted">Review submitted milestone evidence.</p></div><div class="timeline-item"><strong>Unread notifications</strong><p class="muted">Check recent workflow changes.</p></div></div></div></div>`;
  } catch (e) {
    err(el, e);
  }
}
export async function smeDashboard() {
  if (!requireAuth(["SUBCONTRACTOR"])) return;
  mountApp("SME Dashboard");
  const root = qs("#page-root");
  root.innerHTML =
    pageHeader(
      "SME Dashboard",
      "Discover opportunities and monitor your bidding progress.",
      `<a class="btn btn-primary" href="${page("public/opportunities.html")}">Browse opportunities</a>`,
    ) +
    `<div id="sme-stats" class="grid grid-4"></div><div class="grid grid-2" style="margin-top:22px"><section class="card"><div class="row-between"><h2>Open opportunities</h2><a href="${page("public/opportunities.html")}" class="text-primary">View all</a></div><div id="sme-opportunities"></div></section><section class="card"><h2>My recent bids</h2><div id="sme-bids"></div></section></div>`;
  try {
    const [r, o, b] = await Promise.all([
      api.reports.dashboard(),
      api.packages.search({ status: "OPEN", page: 0, size: 4 }),
      api.bids.mine(0, 5),
    ]);
    qs("#sme-stats").innerHTML = [
      ["My Bids", r.totalBids],
      ["Submitted", r.submittedBids],
      ["Active Awards", r.activeAwards],
      ["Pending Milestones", r.pendingMilestones],
    ]
      .map(
        (m, i) =>
          `<div class="card stat"><div><small>${m[0]}</small><strong>${m[1] ?? 0}</strong></div><div class="stat-icon">${["▤", "✎", "✓", "◷"][i]}</div></div>`,
      )
      .join("");
    const ol = listOf(o);
    qs("#sme-opportunities").innerHTML = ol.length
      ? ol
          .map(
            (x) =>
              `<div class="notification"><div class="notification-icon">⌕</div><div><a href="${page("public/opportunity-details.html")}?id=${x.id}"><strong>${esc(x.title)}</strong></a><div class="help">${money(x.budgetMax)} · ${date(x.deadline)}</div></div></div>`,
          )
          .join("")
      : empty("No open packages", "Check again later.");
    const bl = listOf(b);
    qs("#sme-bids").innerHTML = bl.length
      ? bl
          .map(
            (x) =>
              `<div class="notification"><div class="notification-icon">▤</div><div><strong>${esc(x.workPackageTitle || "Bid #" + x.id)}</strong><div>${badge(x.status)} · ${money(x.amount)}</div></div></div>`,
          )
          .join("")
      : empty("No bids yet", "Submit your first bid from an open opportunity.");
  } catch (e) {
    toast(e.message, "error");
  }
}
export async function bidForm() {
  if (!requireAuth(["SUBCONTRACTOR"])) return;
  mountApp("Submit Bid");
  const root = qs("#page-root"),
    workPackageId = idParam("workPackageId"),
    bidId = idParam("id");
  root.innerHTML =
    pageHeader(
      bidId ? "Edit Draft Bid" : "Submit Your Bid",
      "Provide a clear commercial proposal and supporting documents.",
    ) +
    `<div class="grid grid-3"><section class="card" style="grid-column:span 2"><form id="bid-form" class="form-grid"><div class="input-group"><label>Bid amount (OMR)</label><input class="input" type="number" step="0.001" name="amount" required></div><div class="input-group"><label>Delivery days</label><input class="input" type="number" min="1" name="deliveryDays" required></div><div class="input-group span-2"><label>Proposal</label><textarea class="textarea" name="proposalText" required></textarea></div><div class="input-group span-2"><label>Supporting document</label><input class="input" type="file" name="documents"><span class="help">PDF or document up to the backend upload limit.</span></div><div class="input-group"><label>Document type</label><select class="select" name="documentTypes"><option>TECHNICAL_PROPOSAL</option><option>COMPANY_PROFILE</option><option>OTHER</option></select></div><div class="row"><button type="button" class="btn btn-secondary" id="save-draft">Save draft</button><button class="btn btn-primary">Submit bid</button></div></form></section><aside class="card"><h3>Before submitting</h3><ul><li>Confirm company verification.</li><li>Review scope and deadline.</li><li>Use OMR and realistic delivery days.</li><li>A company may have only one active bid per package.</li></ul></aside></div>`;
  if (bidId)
    try {
      const d = await api.bids.get(bidId);
      ["amount", "deliveryDays", "proposalText"].forEach(
        (k) => (qs(`[name=${k}]`).value = d[k] ?? ""),
      );
    } catch (e) {
      toast(e.message, "error");
    }
  const submit = async (draft) => {
    const form = qs("#bid-form"),
      fd = new FormData(form);
    if (!form.documents.files.length) fd.delete("documents");
    try {
      const d = bidId
        ? await api.bids.update(bidId, fd)
        : await api.bids.create(workPackageId, fd, draft);
      if (!draft && d.status === "DRAFT") await api.bids.action(d.id, "submit");
      toast(draft ? "Draft saved" : "Bid submitted");
      location.href = page("subcontractor/my-bids.html");
    } catch (e) {
      toast(e.message, "error");
    }
  };
  qs("#bid-form").onsubmit = (e) => {
    e.preventDefault();
    submit(false);
  };
  qs("#save-draft").onclick = () => submit(true);
}
export async function myBids() {
  ...
  root.innerHTML =
    pageHeader(
      "My Bids",
      "Track drafts, submitted proposals and bid decisions.",
    ) + `<div id="my-bids"></div>`;
  const el = qs("#my-bids");
  loading(el);
  try {
    const l = listOf(await api.bids.mine(0, 50));
    el.innerHTML = l.length
    ...
    qsa(".bid-submit").forEach(
      (b) =>
        (b.onclick = async () => {
          try {
            await api.bids.action(b.dataset.id, "submit");
            toast("Bid submitted");
            location.reload();
          } catch (e) {
            toast(e.message, "error");
    ...
    qsa(".bid-withdraw").forEach(
      (b) =>
        (b.onclick = async () => {
          if (confirm("Withdraw this bid?"))
            try {
              await api.bids.action(b.dataset.id, "withdraw");
              toast("Bid withdrawn");
              location.reload();
            } catch (e) {
              toast(e.message, "error");
...
export async function adminDashboard() {
  ...
  mountApp("Administrator Dashboard");
  const root = qs("#page-root");
  root.innerHTML =
    pageHeader(
      "Platform Administration",
      "Monitor companies and operational activity.",
      `<a class="btn btn-primary" href="${page("admin/verification-queue.html")}">Review companies</a>`,
    ) +
    `<div id="admin-stats" class="grid grid-4"></div><div class="grid grid-2" style="margin-top:22px"><section class="card"><h2>Pending verification</h2><div id="admin-pending"></div></section><section class="card"><h2>Platform priorities</h2><div class="timeline"><div class="timeline-item"><strong>Verify company evidence</strong><p class="muted">Commercial Registration is required before approval.</p></div><div class="timeline-item"><strong>Maintain categories</strong><p class="muted">Keep work package classification clear.</p></div><div class="timeline-item"><strong>Monitor platform activity</strong><p class="muted">Use dashboard totals and notifications.</p></div></div></section></div>`;
  try {
    ...
    qs("#admin-stats").innerHTML = [
      ...
      ["Projects", d.totalProjects],
      ["Packages", d.totalWorkPackages],
      ["Awards", d.activeAwards],
    ]
      .map(
        (m) =>
          `<div class="card stat"><div><small>${m[0]}</small><strong>${m[1] ?? 0}</strong></div></div>`,
      )
      .join("");
    const l = listOf(p);
    qs("#admin-pending").innerHTML = l.length
      ? l
          .slice(0, 6)
          .map(
            (c) =>
              `<div class="notification"><div class="notification-icon">◉</div><div style="flex:1"><strong>${esc(c.legalName)}</strong><div>${badge(c.verificationStatus)} · ${esc(c.crNumber)}</div></div><a class="btn btn-secondary btn-sm" href="${page("admin/verification-queue.html")}?companyId=${c.id}">Review</a></div>`,
          )
          .join("")
      : empty("Queue is clear", "No company is waiting for review.");
  } catch (e) {
    toast(e.message, "error");
  }
}
export async function verificationQueue() {
  if (!requireAuth(["ADMIN"])) return;
  mountApp("Verification Queue");
  const root = qs("#page-root");
  root.innerHTML =
    pageHeader(
      "Company Verification Queue",
      "Review company identity and documents before approval.",
    ) + `<div id="queue"></div><div id="modal"></div>`;
  const el = qs("#queue");
  loading(el);
  try {
    const l = listOf(await api.company.pending());
    el.innerHTML = l.length
      ...
      : empty("No pending companies", "New registrations will appear here.");
    qsa(".review-company").forEach(
      (b) =>
        (b.onclick = async () => {
          const c = await api.company.get(b.dataset.id, 0, "ADMIN");
          let docs = [];
          try {
            docs = listOf(await api.documents.list(c.id));
          } catch {}
          qs("#modal").innerHTML =
          ...
          qs("#close").onclick = () => (qs("#modal").innerHTML = "");
          qs("#decision-form").onsubmit = async (e) => {
            e.preventDefault();
            try {
              await api.company.verify(
              ...
              toast("Verification decision saved");
              qs("#modal").innerHTML = "";
              location.reload();
            } catch (ex) {
              toast(ex.message, "error");
...
export async function categories() {
  if (!requireAuth(["ADMIN"])) return;
  mountApp("Category Management");
  const root = qs("#page-root");
  root.innerHTML =
    pageHeader(
      "Category Management",
      "Maintain classifications used by work packages.",
      '<button class="btn btn-primary" id="new-category">Add category</button>',
    ) + `<div id="categories"></div><div id="modal"></div>`;
  const load = async () => {
    const el = qs("#categories");
    loading(el);
    try {
      const l = listOf(await api.categories.all());
      el.innerHTML = l.length
        ...
        : empty("No categories", "Add the first work category.");
      qsa(".edit-category").forEach(
        (b) => (b.onclick = () => open(b.dataset.id)),
      );
      qsa(".deactivate-category").forEach(
        (b) =>
          (b.onclick = async () => {
            if (confirm("Deactivate category?")) {
              await api.categories.deactivate(b.dataset.id);
              load();
            }
          }),
      );
  ...
  async function open(id) {
    let c = {};
    if (id) c = await api.categories.get(id);
    qs("#modal").innerHTML =
    ...
    qs("#category-form").onsubmit = async (e) => {
      e.preventDefault();
      const b = formObject(e.target);
      b.parentCategoryId = numberOrNull(b.parentCategoryId);
      try {
        id
          ? await api.categories.update(id, b)
          : await api.categories.create({ ...b, active: true });
...
const handlers = {
  ...
  "compare-bids": compareBids,
  reports,
  "sme-dashboard": smeDashboard,
  "bid-form": bidForm,
  "my-bids": myBids,
...
export async function runPage() {
  ...
  if (fn) await fn();
    ...
    document.body.innerHTML =
      '<main class="container section"><h1>Page not found</h1></main>';
}
